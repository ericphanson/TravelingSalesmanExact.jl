...
function test_tour(input, opt; kwargs...)
...
@testset "att48.tsp" begin
    ...
    sym_tour, sym_cost = test_tour(cities, with_optimizer(GLPK.Optimizer); distance = TravelingSalesmanExact.ATT, verbose = true)
    @test sym_cost ≈ 10628

    asym_tour, asym_cost = test_tour(cities, with_optimizer(GLPK.Optimizer); distance = TravelingSalesmanExact.ATT, symmetric = false)
...
@testset "Small random asymmetric" begin
    ...
    t1, c1 = test_tour(cost, with_optimizer(GLPK.Optimizer); verbose = true)
    t2, c2 = test_tour(cost, with_optimizer(GLPK.Optimizer); verbose = false)
    @test c1 ≈ c2
...
@testset "Small random symmetric" begin
    ...
    t4, c4 = test_tour(cost_sym, with_optimizer(GLPK.Optimizer); verbose = false)
    t5, c5 = test_tour(cost_sym, with_optimizer(GLPK.Optimizer); symmetric = true, verbose = false)
    t6, c6 = test_tour(cost_sym, with_optimizer(GLPK.Optimizer); symmetric = false, verbose = false)
    @test c4 ≈ c5    
    @test c5 ≈ c6
end
...
@testset "Small random cities" begin
    ...
    t7, c7 = test_tour(cities, with_optimizer(GLPK.Optimizer); verbose = false)
    t8, c8 = @inferred test_tour(cities, with_optimizer(GLPK.Optimizer); verbose = false, symmetric = false)
    cost = [ TravelingSalesmanExact.euclidean_distance(c1, c2) for c1 in cities, c2 in cities ]
    t9, c9 = test_tour(cost, with_optimizer(GLPK.Optimizer); verbose = false, symmetric = false)
    t10, c10 = test_tour(cost, with_optimizer(GLPK.Optimizer); verbose = false, symmetric = true)
    @test c7 ≈ c8
    @test c8 ≈ c9
    @test c9 ≈ c10
    @test c8 isa Float64
...
@testset "Exceptions" begin
    cost = rand(5,4)
    @test_throws ArgumentError test_tour(cost, with_optimizer(GLPK.Optimizer))
end